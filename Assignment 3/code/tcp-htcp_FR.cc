...
TcpHtcp::TcpHtcp(const TcpHtcp& sock)
    : TcpNewReno(sock),
      ...
      m_recoverySeq(sock.m_recoverySeq)
{
    NS_LOG_FUNCTION(this);
}
...
//ADDED
bool
TcpHtcp::AllRetransmittedAcked(Ptr<TcpSocketState> tcb)
{
  
  SequenceNumber32 ackSeq = tcb->m_lastAckedSeq;
  return ackSeq >= m_recoverySeq;
}

//ADDED
void
TcpHtcp::CongestionStateSet(Ptr<TcpSocketState> tcb, TcpSocketState::TcpCongState_t newState)
{
  NS_LOG_FUNCTION(this << tcb << newState);

  if (newState == TcpSocketState::CA_RECOVERY)
  {
    NS_LOG_INFO("Entering Fast Recovery");
    // Setting recovery flag
    m_inRecovery = true;
    // Recording the sequence number where recovery started
    m_recoverySeq = tcb->m_lastAckedSeq + 1;
  }
...
//ADDED
void
TcpHtcp::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  if (m_inRecovery)
  {
    if (segmentsAcked > 0)
    {
      ...
      NS_LOG_DEBUG("Fast Recovery: Inflated cwnd to " << tcb->m_cWnd);
    }

    // Check if all retransmitted data has been acknowledged
    if (AllRetransmittedAcked(tcb))
    {
      tcb->m_cWnd = tcb->m_ssThresh;
      tcb->m_congState = TcpSocketState::CA_OPEN;
      m_inRecovery = false;
      NS_LOG_INFO("Fast Recovery Complete: cwnd reset to " << tcb->m_cWnd);
    }
