...
class TcpHtcp : public TcpNewReno
{
  public:
    ...
    void IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) override;    //ADDED
    void CongestionStateSet(Ptr<TcpSocketState> tcb, TcpSocketState::TcpCongState_t newState) override; //ADDED
  protected:
  ...
  private:
    ...
    bool AllRetransmittedAcked(Ptr<TcpSocketState> tcb); //ADDED (Determines if all retransmitted packets have been acknowledged, signaling the end of recovery.)
    ...
    bool m_inRecovery;               //!< Flag indicating if in Fast Recovery (ADDED)
    SequenceNumber32 m_recoverySeq;   //!< Sequence number at recovery (ADDED)
};
